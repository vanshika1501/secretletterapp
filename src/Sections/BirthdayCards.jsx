...
export default function BirthdaySecrets() {
  ...
  const handleGroupPinSubmit = (groupKey) => {
    ...
    if (entered === expected) {
      if (!unlockedGroups.includes(groupKey)) {
        ...
        // Smoothly keep scroll position stable
        setTimeout(() => window.scrollTo({ top: window.scrollY, behavior: "smooth" }), 50);
      }
    } else {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-start bg-[#f9f5f0] overflow-x-hidden scroll-smooth p-6 sm:p-10">
          ...
          {groupsInUse.map(groupKey => {
            ...
            return (
              <motion.div layout key={groupKey}>
                ...
                <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center">
                  {groupCards.map(friend => (
                    <motion.div
                      layout
                      key={friend.id}
                      className={`relative bg-[#fffaf3] shadow-md rounded-2xl p-5 sm:p-6 border w-64 sm:w-72 mx-auto border-[#d6c2b5] overflow-hidden text-center min-h-[230px] ${
      ...
      <AnimatePresence>
        {unlockedFriend && (
          ...
          >
            <motion.div
              className="relative bg-[#fff9f4] shadow-2xl w-full sm:w-4/5 md:w-2/3 max-h-[90vh] overflow-y-auto p-6 sm:p-10 flex flex-col items-center rounded-2xl"
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
              exit={{ scale: 0.9 }}
            >
              ...
              {unlockedFriend.image.endsWith(".pdf") ? (
                <div className="w-full h-[75vh] rounded-xl border border-[#e5d2c2] overflow-hidden">
