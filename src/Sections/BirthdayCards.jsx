...
export default function BirthdaySecrets() {
  ...
  const groupPins = {
    group1: "1111", // Cards 1–5
    group2: "2222", // Cards 6–10
    group3: "3333", // Cards 11–15
  };
  ...
  const friends = [
    {
      id: 1,
      name: "Starts with 8, ends with 9, opens the door to the new world where we have siblings to find.",
      password: "816274169",
      group: "group1",
    },
    {
      id: 2,
      name: "Reshu didi bht achi hai, bo mujhe apne ______ se khelne detin hai.",
      password: "bholu",
      group: "group1",
    },
    {
      id: 3,
      name: "Reshu tum mera kaunsa gaya hua gaana loop pe sunti ho?",
      password: "ande",
      group: "group1",
    },
    {
      id: 4,
      name: "I am the repetitive meeting place that always makes you grin, And the one place I hate where you constantly drag me in. Where am I?",
      password: "dainikbhaskarmall",
      group: "group1",
    },
    {
      id: 5,
      name: "Chhoti thi height, badi thi baat, Wahi se shuru hui dosti ki shuruaat♥️ Bata kis class?",
      password: "5",
      group: "group1",
    },
    ...
    { id: 10, name: "Paras bhaiya", password: "epsilon", group: "group2" },

    // group 3
    { id: 11, name: "Rohit bhaiya", password: "oneone", group: "group3" },
    { id: 12, name: "Aadi bhaiya", password: "twotwo", group: "group3" },
    { id: 13, name: "Ansh bhaiya", password: "threethree", group: "group3" },
    { id: 14, name: "Mummy", password: "fourfour", group: "group3" },
    { id: 15, name: "Dhruvi bhaiya", password: "fivefive", group: "group3" },
  ];
  ...
  const handleUnlock = (id) => {
    ...
    if (!friend) return;
    const enteredPassword = (enteredPasswords[id] || "").toLowerCase();
  ...
  const handleGroupPinChange = (groupKey, value) => {
    setEnteredGroupPins((prev) => ({ ...prev, [groupKey]: value }));
  };
  ...
  const handleGroupPinSubmit = (groupKey) => {
    ...
    if (entered === expected) {
      if (!unlockedGroups.includes(groupKey)) {
        setUnlockedGroups((prev) => [...prev, groupKey]);
      }
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-start bg-[#f9f5f0] overflow-x-hidden p-6 sm:p-10">
      ...
      <div className="w-full max-w-6xl z-10">
        <h2 className="text-3xl sm:text-5xl font-shifty text-[#705c53] mb-2 tracking-wide text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {groupsInUse.map((groupKey) => (
            <div key={groupKey} className="bg-[#fffaf3] border border-[#d6c2b5] p-4 rounded-2xl shadow-md flex flex-col items-center">
              ...
              <div key={groupKey}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {groupCards.map((friend) => (
                    <motion.div
                      key={friend.id}
                      className={`relative bg-[#fffaf3] shadow-md rounded-2xl p-5 sm:p-6 border border-[#d6c2b5] transition-transform overflow-hidden ${isGroupUnlocked ? "" : "blur-sm pointer-events-none"}`}
...
<div className="flex justify-center">
  ...
  {unlockedFriend && (
    ...
    >
      <motion.div
        className="relative bg-[#fff9f4] shadow-2xl w-full max-w-3xl h-[95vh] overflow-y-auto p-6 sm:p-10 flex flex-col items-center rounded-2xl"
        initial={{ scale: 0.9 }}
        animate={{ scale: 1 }}
        exit={{ scale: 0.9 }}
      >
        ...
        {unlockedFriend.image.endsWith(".pdf") ? (
          <div className="w-full h-full rounded-xl border border-[#e5d2c2] overflow-hidden">
          ...
          <img
            ...
            alt="Letter"
            className="rounded-xl mb-4 sm:mb-6 max-w-full max-h-full object-contain shadow-lg border border-[#e5d2c2]"
          />
        )}
